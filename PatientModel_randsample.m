...
%% EVALUATE ACCURACIES OF RANDOM FORESTS
...
for y = 1:length(IDs)
    ...
    %Generate and collect weights
    weights_temp = zeros(size(features_p,1),1);
    %weights_temp(:) = acc_new(y); %by patient
    weights_temp = diagonal_new(codesTrue_p); %by patient and activity
    weights_acc = [weights_acc; weights_temp];
    
    %Collect features and codesTrue
    features_all = [features_all; features_p];
    codesTrue_all = [codesTrue_all codesTrue_p];
end
disp('Posteriors generated for main and new sessions.')

%% FORESTS: SUMMARY TABLE
...
%% RANDOMLY SAMPLE
%Cycle through each activity and randomly sample (weighted)
features_w = [];
codesTrue_w = [];
ind_rand_all = [];
for k = 1:length(states)
    ind = find(codesTrue_all == k);
    ind_rand = randsample(ind,length(ind),true,weights_acc(ind));
    features_w = [features_w; features_all(ind_rand,:)]; 
    codesTrue_w = [codesTrue_w codesTrue_all(ind_rand)];
    ind_rand_all = [ind_rand_all ind_rand];
end

%Plot histogram of weights (accuracies)
figure;
subplot(2,1,1)
histogram(weights_acc); %raw weights
subplot(2,1,2)
histogram(weights_acc(ind_rand_all)) %randomly sampled weights

%% LAYER 2: TRAIN CLASSIFIER + GENERATE FINAL ACTIVITY PREDICTIONS
%Random Forest (RF)
ntrees = 10;
RFmodel = TreeBagger(ntrees,features_w,codesTrue_w,'OOBVarImp',OOBVarImp,'Options',opts_ag);
